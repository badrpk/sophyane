...
class FakeOwner:
    ...
    def emit(self, role, text):
        self.events.append(("emit", role, text))


def test_multiline_request_is_preserved_as_one_value():
    owner = FakeOwner(
        [
            "Create a competitive coordinator.",
            "",
            "Requirements:",
            "- two candidates",
            "- held-out gates",
    ...
    result = _collect_multiline_request(owner)
    ...
    assert any(
        event[0] == "emit"
        and "finish with /end" in event[2]
        for event in owner.events
    )


def test_cancel_returns_none_without_submitting_lines():
    owner = FakeOwner(
        [
            "unfinished task",
            "/cancel",
    ...
    assert _collect_multiline_request(owner) is None
    assert any(
        event[0] == "emit"
        and "cancelled" in event[2].casefold()
        for event in owner.events
    )


def test_empty_collection_returns_usage():
    owner = FakeOwner(["/end"])

    assert _collect_multiline_request(owner) is None
    assert any(
        event[0] == "emit"
        and "Usage:" in event[2]
        for event in owner.events
    )
