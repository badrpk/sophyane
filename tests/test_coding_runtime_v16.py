...
def test_repository_index_symbols_search_and_context(tmp_path: Path) -> None:
    ...
    (tmp_path / "test_app.py").write_text("def test_total():\n    assert True\n", encoding="utf-8")
    snapshot = RepositoryIndex(tmp_path).build()
    assert "app.py" in snapshot.files
    assert "test_app.py" in snapshot.tests
    assert any(item.name == "Inventory" and item.kind == "class" for item in snapshot.symbols)
...
def test_precise_patch_preserves_unrelated_content(tmp_path: Path) -> None:
    ...
    evidence = PatchEngine(tmp_path).replace_exact("module.py", "return 1", "return 42")
...
def test_task_queue_respects_dependencies() -> None:
    queue = TaskQueue([
        ...
        {"id": "api", "objective": "Create API", "depends_on": ["schema"]},
    ])
...
def test_mechanical_verifier_is_authoritative(tmp_path: Path) -> None:
    ...
    failed = verifier.verify([{"type": "file_exists", "path": "missing.txt"}])
    assert not failed["passed"]


def test_dependency_advisor_extracts_missing_module() -> None:
    result = DependencyAdvisor.diagnose("ModuleNotFoundError: No module named 'requests'")
...
def test_batched_write_run_and_verify(tmp_path: Path) -> None:
    ...
    def backend(prompt: str, system: str) -> str:
        ...
        if "mechanical" in system.lower() or "Accept deterministic" in system:
            calls["verifier"] += 1
            observations = payload["latest_observation"].get("observations", [])
            ...
            return json.dumps({
                ...
                "next_instruction": "",
                "final_answer": "Created and tested calculator.py in one batch.",
            })
        calls["planner"] += 1
        return json.dumps({
            "objective": "Create and test calculator",
            "success_criteria": ["calculator.py exists", "script exits zero"],
            "action": {
                "type": "batch",
                "actions": [
                    {"type": "write_file", "path": "calculator.py", "content": "assert 2 + 2 == 4\nprint('ok')\n"},
                    {"type": "run_command", "argv": [sys.executable, "calculator.py"]},
                ],
            },
            "rationale": "Independent safe steps can execute together.",
        })
    ...
    assert result.execution["repository"]["file_count"] if "file_count" in result.execution["repository"] else result.execution["repository"]["files"]


def test_batch_stops_on_failure_and_reports_dependency(tmp_path: Path) -> None:
    def backend(prompt: str, system: str) -> str:
        if "Accept deterministic" in system:
            return json.dumps({
                "goal_met": False,
                "confidence": 1,
                "missing_requirements": ["dependency missing"],
                "next_instruction": "Install or replace dependency safely",
                "final_answer": "",
            })
        return json.dumps({
            "objective": "diagnose",
            ...
            "action": {
                "type": "run_command",
                "argv": [sys.executable, "-c", "import package_that_does_not_exist_123"],
            },
    ...
    assert observation["dependency_diagnosis"]["kind"] == "missing_python_dependency"


def test_patch_engine_rejects_ambiguous_match(tmp_path: Path) -> None:
    (tmp_path / "x.py").write_text("x = 1\nx = 1\n", encoding="utf-8")
    with pytest.raises(Exception):
        PatchEngine(tmp_path).replace_exact("x.py", "x = 1", "x = 2")
