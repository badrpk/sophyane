...
VERSION='9.0.7';ROOT=Path.home()/'.local/state/sophyane';DB=ROOT/'sophyane.sqlite3';LOG=ROOT/'events.jsonl';CPS=ROOT/'checkpoints';BASE=Path.home()/'.local/share/sophyane';CORE=BASE/'sophyane-core-9.0.5.py';CORE_URL='https://raw.githubusercontent.com/badrpk/sophyane/e55a45db92f8a719f514c21b2cae441db527ff4d/sophyane-9.0.5.py';CORE_BLOB='2cee1f67c69d85fed2f5d473be35a93e8ca5b0c2';TERM={'completed','failed','cancelled'}
...
def conn():
 ...
 if 'tasks' in names:
  ...
  if not {'id','project_id','title','command','deps','status','attempts','max_attempts','timeout','verify','stdout','stderr','exit_code','latency_ms','created','updated'}.issubset(cols):
 ...
 if 'projects' in names:
  ...
  if not {'id','name','workspace','status','created','updated'}.issubset(cols):
 ...
 if 'checkpoints' in names:
  ...
  if not {'id','project_id','path','sha256','created'}.issubset(cols):
   stamp=int(time.time());c.execute(f'ALTER TABLE checkpoints RENAME TO checkpoints_legacy_{stamp}')
...
def new(name,workspace=None):
 ...
 with conn() as c:c.execute('INSERT INTO projects(id,name,workspace,status,created,updated) VALUES(?,?,?,?,?,?)',(pid,name,str(w),'ready',n,n));c.commit()
...
def add(pid,title,command,deps,retries,timeout,verify):
 ...
 with conn() as c:
  ...
  c.execute('''INSERT INTO tasks(id,project_id,title,command,deps,status,attempts,max_attempts,timeout,verify,stdout,stderr,exit_code,latency_ms,created,updated) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(tid,pid,title,command,json.dumps(deps),'pending',0,retries,timeout,verify,'','',None,0.0,n,n));rs=c.execute('SELECT * FROM tasks WHERE project_id=?',(pid,)).fetchall()
...
def checkpoint(pid,reason):
 ...
 with conn() as c:c.execute('INSERT INTO checkpoints(id,project_id,path,sha256,created) VALUES(?,?,?,?,?)',(uuid.uuid4().hex,pid,str(p),sha,time.time()));c.commit()
