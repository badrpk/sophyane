...
- Hugging Face GGUF open-model path when llama.cpp cannot install or run
...
- Disk reclaim of failed legacy local-runtime extracts before HF bootstrap

### Verified

- Freeing `legacy-runtime-extract` recovered ~1.6GB on constrained Crostini
...
- Hardware-aware local open-model bootstrap (`local_runtime.py`): profile RAM/disk, install llama.cpp, pull tier-fit model, warm-up, auto-switch config
...
- `llm.json` fallback order prefers recovered llama.cpp runtime after auto-promotion
